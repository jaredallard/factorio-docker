...
type Config struct {
	...
	// https://wiki.factorio.com/Multiplayer#How_to_list_a_server-hosted_game_on_the_matching_server
	Token string `env:"TOKEN,unset" yaml:"token"`

	// Factocord is configuration for running Factocord.
	Factocord Factocord `envPrefix:"FACTOCORD_" yaml:"factocord"`
}
...
type Factocord struct {
	...
	DiscordToken string `env:"DISCORD_TOKEN,unset" yaml:"discord_token"`

	// DiscordChannelID is the ID of the Discord channel to send messages to.
	DiscordChannelID string `env:"DISCORD_CHANNEL_ID" yaml:"discord_channel_id"`

	// DiscordUserColors determines whether to use user colors in Discord messages.
	DiscordUserColors bool `env:"DISCORD_USER_COLORS" default:"true" yaml:"discord_user_colors"`
}
